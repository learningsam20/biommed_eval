...
class BM25LexicalIndex:
    """Okapi BM25 over whitespace-tokenized passages. Persisted as a pickle."""
    ...
    def build(self, ids: List[int], texts: List[str]) -> None:
        """Fit BM25 on the corpus. `ids[i]` is the passage ID for `texts[i]`."""
        self._ids = ids
        self._texts = texts
    ...
    def query(self, query: str, top_k: int = 50) -> List[Tuple[int, float]]:
        """Return ``(passage_id, bm25_score)`` pairs, highest first."""
    ...
    def save(self, path: str) -> None:
        """Pickle ids/texts/k1/b. Load rebuilds the Okapi index from texts."""
    ...
    def load(cls, path: str) -> "BM25LexicalIndex":
        """Restore a pickle written by ``save`` and rebuild BM25 in memory."""
